...
function CoursePanel({ level }: LocProps) {
  ...
  const [courseModules, setCourseModules] = useState<{
    ...
    finishedLessons?: {
      lessonId: number;
      progressStatus: "good" | "very good" | "perfect";
    }[];
  }>({ modules: [] });
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/course_modules", {
        params: { level: Level[level], email: user },
      })
      .then((res) => {
        const { lessons: lessonsRetrived, modules: modulesRetrieved } =
          res.data;

        let modules = modulesRetrieved.map(
          (m: { id: number; title: string }) => {
            return {
              title: m.title,
              lessons: lessonsRetrived.filter(
                (l: lessonItem) => l.moduleId === m.id,
              ),
            };
          },
        );
  ...
  const courseModulesJSX = useMemo(() => {
    return courseModules.modules.map((c, index) => {
      const Lessons = courseModules.finishedLessons
        ? c.lessons.map((l) => {
            const statusLesson = courseModules.finishedLessons!.find(
              (f) => f.lessonId === l.id,
            );
            return {
              ...l,
              status: !statusLesson
                ? IconStatus.NO_PROGRESS
                : statusLesson.progressStatus === "good"
                ? IconStatus.STAR_1
                : statusLesson.progressStatus === "very good"
                ? IconStatus.STAR_2
                : IconStatus.STAR_3,
            };
          })
      ...
      return Lessons.length > 0 ? (
        <CourseModule
          title={new StringMaxLength(c.title, COURSE_TITLE_LENGTH)}
          key={index}
          index={index}
  ...
  return (
    <div className={`course-plan course-plan--${Level[level]}`}>
      <div className="flex-column--centered">{courseModulesJSX}</div>
    </div>
  );
}
