...
function Contacts() {
  ...
  function submit(event:React.FormEvent<HTMLFormElement>){
    ...
    axios.post('http://localhost:5000/contacts', {email:emailAdress, topic:topic, message:message}).then((response)=>{
      const {status} = response.data;
      if(status === 'sent'){
        topicRef.current!.value = '';
        messageRef.current!.value = '';
        setNotification({isShown: true, message:'Mesaj trimis !', type:NotificationType.SUCCESS});
      }else{
        setNotification({isShown: true, message:'Eroare, încercați mai târziu !', type:NotificationType.ERROR});
      }
    }).catch(()=>{setNotification({isShown: true, message:'Eroare, încercați mai târziu !', type:NotificationType.ERROR});});    
  }
