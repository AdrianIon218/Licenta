...
function ResetPassCode() {
    ...
    const [notification, setNotification] = useState({isShown:false, messsage:'', type:NotificationType.NO_TYPE});
    ...
    const onSubmit = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>)=>{
      ...
      if(isCodeExpired){
        ...
        axios.post('http://localhost:5000/login/verify_code_reset',{email:emailToReset,enteredCode:enteredCode}).then((response)=>{
          ...
          if(isCorrect){
            ...
            if(status){
              setNotification({isShown:true, messsage:'Codul este incorect !',type:NotificationType.WARNING});
            }else{
              setNotification({isShown:true, messsage:'Eroare, încercați mai târziu !', type:NotificationType.ERROR});
            }
          }
        }).catch(()=>{
          setNotification({isShown:true, messsage:'Eroare, încercați mai târziu !', type:NotificationType.ERROR});
    ...
    return (<>
    {notification.isShown && <Notification message={notification.messsage} type={notification.type} 
      deleteNotification={()=>{setNotification({isShown:false, messsage:'', type:NotificationType.NO_TYPE});}} />}
        ...
        <div className="box-mountain-bg__form">
          <form className="forn">
            <div className="u-center-text u-margin-bottom-medium">
              <h2 className="heading-secondary" style={{textTransform:'none'}}>
                {isCodeExpired ? `Codul trimis la ${emailToReset} a expirat!`:`Introduceți codul trimis la adresa ${emailToReset}`}
              </h2>
              <p style={{fontSize:'1rem',fontWeight:800}} hidden={isCodeExpired}>
                Codul expiră în &nbsp;<CountDownTime time={expireCodeTime} codeExpire={()=>setCodeExpire(true)}/>
              </p>
            </div>
            
            <div className="form__group u-margin-bottom-intermediate" hidden={isCodeExpired}>
