...
enum ExerciseType { MUTIPLE_CHOICE, WRITE_wORD, CHOOSE_TRANSLATION};
  
function TestLesson(props:LocProps) {
  const [exerciseInfo, setExerciseInfo] = useState({progressBar:0, score: 0, wordIndex: 0});
  ...
  const commonMethodesWords = {
    correctAnswear: ()=>{ 
      setExerciseInfo(oldVal=>{
        ...
        return ({progressBar: newProgress, score:oldVal.score + 1,
            wordIndex: oldVal.wordIndex < props.unkwonWords.length ? oldVal.wordIndex + 1 : props.unkwonWords.length})
        });
  ...
  useEffect(()=>{
    ...
    setExercisesWordJSX(props.unkwonWords.map((word, index)=>{
      const currentExerciseType =  index % 3;
      switch(currentExerciseType){
        case ExerciseType.WRITE_wORD:
          return (<WriteWord key={index} {...word} {...commonMethodesWords} skipExercise={skipHandler} />);
        case ExerciseType.MUTIPLE_CHOICE:
          return (<MultipleChoice key={index} {...word} {...commonMethodesWords} wordNames={wordNames} />);
        default:
          return (<ChooseTranslation key={index} {...word} {...commonMethodesWords} wordsTranslation={wordsTranslations} />);
      }
    }));
  },[props.unkwonWords]);
  ...
  return (
    <div className='new_words_ex'>
      ...
      { exerciseInfo.wordIndex < props.unkwonWords.length && exercisesWordJSX[exerciseInfo.wordIndex] }
      { exerciseInfo.wordIndex === props.unkwonWords.length && 
        <GrammerExercises currentScore={exerciseInfo.score} previousExercisesNum={props.unkwonWords.length}
          lessonId={props.lessonId} addProgress={props.setProgressBar} toFinish={props.toFinish}/>}
    </div>);
}
