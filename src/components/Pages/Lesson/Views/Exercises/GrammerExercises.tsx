...
function GrammerExercises(props:LocProps) {
    ...
    useEffect(()=>{
        axios.post("http://localhost:5000/lessons/grammer/getExercises", {lessonId: props.lessonId}).then(response => { 
          const exercises:{type:'true_false'|'match_choices'|'complete_sentence'|'order_words', structure:any}[] = response.data;
          ...
          setExerciseComps(exercises.map((exercise,index)=>{
            ...
            return (<React.Fragment key={index}>
                { exercise.type === 'true_false' && <TrueOrFalse {...commonStructure} />}
                { exercise.type === 'complete_sentence' && <CompleteSentence {...commonStructure} />}
                { exercise.type === 'match_choices' && <MatchChoices {...commonStructure} /> }
                { exercise.type === 'order_words' && <OrderWords {...commonStructure} />}
                </React.Fragment>);
          }));
        });
    },[props.lessonId]);
    ...
    useEffect(()=>{
      ...
      if(generalState.currentExIndex > 0 && generalState.currentExIndex === numOfExercises){
        if(props.currentScore && props.previousExercisesNum){
          ...
          const statusResult = totalScore > numOfTotalExercises/2 ? IconStatus.STAR_3 : 
                               totalScore > numOfTotalExercises/3 ? IconStatus.STAR_2 : IconStatus.STAR_1;
          props.toFinish(statusResult);
        }
        else{
          const statusResult = generalState.score > numOfExercises/2 ? IconStatus.STAR_3 : 
                             generalState.score > numOfExercises/3 ? IconStatus.STAR_2 : IconStatus.STAR_1;
          props.toFinish(statusResult);
        }
      }
    },[generalState])
