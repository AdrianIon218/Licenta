...
function WordsExercises(props:LocProps) {
  const reducer = (state:any, action:{type:ACTION, progressRatio:number})=>{
    switch (action.type) {
      case ACTION.CORRECT_ANSWEAR:
        props.setProgressBar(state.progressBar + action.progressRatio);
        return {progressBar: state.progressBar + action.progressRatio, score: state.score + 1, wordIndex: state.wordIndex + 1};
  ...
  const [exerciseInfo, dispatch] = useReducer(reducer, {progressBar:50, score:props.score ?? 0, wordIndex: 0});
  ...
  const exerciseJSX = useMemo(()=> {
    ...
    return props.unkwonWords.map((word, index)=>{
      const currentExerciseType = lessonType === 'pronunciation' ? index % 4 : index % 3;

      if(currentExerciseType === ExerciseType.WRITE_wORD){
        return (<WriteWord key={index} {...word} {...commonMethodes} skipExercise={skipHandler} />);
      }
      ...
      return (<WordRecord key={index} {...word} {...commonMethodes} skip={skipHandler} stopNotification={()=> setNotification(false)} />);
    })
  },[props.unkwonWords]);

  useEffect(()=>{
    if(exerciseInfo.wordIndex === props.unkwonWords.length) {
      const numOfExercises = lessonType === 'pronunciation' ? 2*numWords : numWords;
      const status = exerciseInfo.score <= numOfExercises/3 ? IconStatus.STAR_1:
                     exerciseInfo.score <= numOfExercises/2 ? IconStatus.STAR_2: IconStatus.STAR_3;
