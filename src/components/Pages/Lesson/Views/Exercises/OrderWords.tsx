...
function OrderWords(props: LocProps) {
  ...
  const sentenceParts = props.structure.words.map((word, index) => {
    return (
      <span
        className={`block-transparent u-center-text ${
          results.isShown
            ? word === choicesMade[index]
              ? "backgd-green"
              : "backgd-red"
            : ""
        }`}
  ...
  const choicesShuffled = useMemo(
    () => [...props.structure.words].sort((a, b) => 0.5 - Math.random()),
    [],
  );
  const choiceElements = choicesShuffled.map((word, index) => {
    return (
      <button
        ...
        value={word}
        disabled={choicesMade.includes(word)}
      >
        {word}
      </button>
    );
  ...
  return (
    <div className="flex-column--centered exercise">
      <h3 className="u-center-text">
        Ordonează cuvintele astfel încât propoziția să fie corectă !
      </h3>
      ...
      {results.isShown && !results.isCorrect && (
        <div className="choices-box correct-choices flex-column--centered">
