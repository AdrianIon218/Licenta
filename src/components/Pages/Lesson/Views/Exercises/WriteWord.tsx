...
function WriteWord(props: LocProps) {
  ...
  const testAnswear = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const userAnswear = removePunctuation(
      answearRef.current!.value.toLowerCase(),
    );
  ...
  const umloutsBtns = useMemo(() => {
    return ["ä", "ö", "ü", "ß"].map((umloat, index) => (
      <button type="button" onClick={() => addUmlout(umloat)} key={index}>
        {umloat}
      </button>
    ));
  }, []);
  ...
  return (
    <>
      ...
      {countMistakes >= 2 && (
        <button className="btn-skip" onClick={props.skipExercise}>
          Skip
        </button>
      )}
      <div className="flex-column--centered word__container">
        ...
        <form
          onSubmit={testAnswear}
          className="flex-column--centered write_word__form"
        >
          <input
            type="text"
            className="flex-element u-center-text"
            maxLength={correctAnswear.length}
            minLength={2}
            ref={answearRef}
            autoFocus
          />
          ...
          <button className="submitBtn u-center-text">
