...
function ShowLessonResult(props: LocProps) {
  ...
  useEffect(() => {
    ...
    if (user) {
      axios
        .post("http://localhost:5000/lessons/addProgress", {
          email: user,
          lessonId: lessonId,
          status: props.statusLesson,
        })
        .then((response) => {
      ...
      <div className="result-stars">
        <i
          className={`fa fa-star ${
            props.statusLesson !== IconStatus.NO_PROGRESS &&
            "course-status--checked"
          }`}
        />
        <i
          className={`fa fa-star ${
            props.statusLesson === IconStatus.STAR_3 && "course-status--checked"
          }`}
        />
        <i
          className={`fa fa-star ${
            props.statusLesson > IconStatus.STAR_1 && "course-status--checked"
          }`}
        />
      </div>
      <button className="btn-gradient-cyan" onClick={() => navigate(-1)}>
        {" "}
        Înapoi la lecții
      </button>
    </div>
