        ...
        if(status){
           ...
           setAvailableCourse(oldVal=>{
            ...
            oldVal.forEach((element, indexElement)=>{
              const index = courses.findIndex((course:CourseResponse)=> course.level === element.heading);
              if(index !== -1){
                newVal[indexElement].details.words = courses[index].wordsNum;
                newVal[indexElement].details.lessonsNumber = courses[index].lessonsNum;
                newVal[indexElement].details.testsNumber = courses[index].testsNum;
                newVal[indexElement].estimationWeeks = Math.ceil((courses[index].lessonsNum/2)/7);
              }
            })
            return newVal;
          });
