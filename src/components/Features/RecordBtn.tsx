...
function RecordBtn(props: LocProps) {
  ...
  const timeToRecord =
    props.textToLookFor.length < 10
      ? 3
      : props.textToLookFor.length < 20
      ? 4
      : 7;
  const textToLookFor = removeUmlauts(
    removePunctuation(props.textToLookFor.toLowerCase()),
  );
  ...
  return (
    <button
      className={`play-btn btn-record ${
        isListening === true ? "btn-recording" : ""
      }`}
      onClick={startRecord}
    >
